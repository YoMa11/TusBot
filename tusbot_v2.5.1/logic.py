...
__file_version__ = "logic.py@1"  # added 2025-08-30 00:15
...
def _extract_price(text: str) -> int | None:
    """Extract price like $555 / 555$ / ₪370 / 370₪ or 'מחיר' followed by number.
    Ignores dates (dd/mm) and day numbers.
    """
    if not text: return None
    # prefer patterns with currency sign
    m = _re2.search(r'(?:\$|₪)\s*([0-9]{2,5})', text)
    if not m:
        m = _re2.search(r'([0-9]{2,5})\s*(?:\$|₪)', text)
    if not m:
        # Hebrew word 'מחיר' then number
        m = _re2.search(r'מחיר[^0-9]{0,6}([0-9]{2,5})', text)
    if not m:
        return None
    val = int(m.group(1))
    # sanity check
    if val < 50:  # avoid 0/1/26 from dates/days
        return None
    return val
...
USD_TO_ILS = float(getattr(cfg, "USD_TO_ILS", 3.7))  # אפשר לכוון ב-config.py
