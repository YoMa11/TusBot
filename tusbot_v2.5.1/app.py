...
# === Versioned root & import lockdown ===
...
# Force DB relative to this versioned directory
try:
    import config as cfg
    try:
        from pathlib import Path as _P
        cfg.DB_PATH = str(_P(__file__).resolve().parent / "flights.db")
    except Exception:
        pass
except Exception as _e:
    pass

def _verify_origin(mod_name: str, expected_root: Path = APP_ROOT) -> Path:
    m = importlib.import_module(mod_name)
    p = Path(getattr(m, "__file__", "")).resolve()
    if p == expected_root / f"{mod_name}.py" or expected_root in p.parents:
        return p
    raise RuntimeError(f"❌ {mod_name} נטען מ-{p} ולא מתוך {expected_root}")

try:
    loaded = {m: str(_verify_origin(m)) for m in ("config","db","handlers","logic","telegram_view","utils")}
    logging.basicConfig(level=logging.INFO)
    logging.info("✅ Modules origin OK: %s", loaded)
    vfile = APP_ROOT / "VERSION"
    if vfile.exists():
        logging.info("📦 VERSION: %s", vfile.read_text(encoding="utf-8").strip())
except Exception as e:
    raise
# === End lockdown ===
__file_version__ = "app.py@1"  # added 2025-08-30 00:15
