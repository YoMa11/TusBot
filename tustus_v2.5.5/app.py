...
__file_version__ = "app.py@tustus_v2.5.5"  # updated 2025-08-30 18:58  # added 2025-08-30 18:21
...
async def monitor_job(app, conn):
    """Unified monitor tick that defers to logic.py.
    Tries several known function names (sync/async), logs and no-ops if none found.
    """
    try:
        candidates = [
            "monitor_job",
        ...
        # Prefer async if exists
        for name in candidates:
            fn = getattr(lg, name, None)
            if fn:
                try:
                    ...
                    # maybe signature (app, conn) or (conn, app)
                    try:
                        if inspect.iscoroutinefunction(fn):
                            return await fn(app, conn)
                        else:
                            return fn(app, conn)
                    except TypeError:
                        try:
                            ...
                            raise e3
        logger.warning("monitor_job: no suitable function found in logic.py; skipping tick.")
    except Exception:
        logger.exception("monitor_job: exception while executing logic")
