...
async def run_monitor(conn, app):
    ...
    Builds a minimal PTB-like context so monitor_job can rely on context.application.
    """
    ...
    # בונים context מינימלי בסגנון PTB
    ctx = types.SimpleNamespace(
        application=app,
        bot=getattr(app, "bot", None),
        job_queue=getattr(app, "job_queue", None),
        # אם monitor_job ירצה גישה ל־DB דרך הקונטקסט:
        db_conn=conn,
    )

    try:
        if inspect.iscoroutinefunction(fn):
            # קודם כל נסה עם context (ככה monitor_job נכתב)
            try:
                return await fn(ctx)
            except TypeError:
                # נסיונות גיבוי לחתימות חריגות
                try:
                    return await fn(app)
                except TypeError:
                    try:
