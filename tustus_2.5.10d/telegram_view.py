...
def price_menu_kb(conn: sqlite3.Connection, prefs: dict, page: int = 1, page_size: int = 12) -> InlineKeyboardMarkup:
    ...
    cur = conn.execute("SELECT DISTINCT price FROM flights WHERE price IS NOT NULL AND price >= 50 ORDER BY price")
...
# --- tustus_2.5.10d: destination keyboard ---
from telegram import InlineKeyboardMarkup, InlineKeyboardButton
def build_dests_kb(dest_names, include_select_all=True, selected_all=False):
    rows = []
    for d in dest_names:
        rows.append([InlineKeyboardButton(f"📍 {d}", callback_data=f"dest:{d}")])
    if include_select_all:
        label = "בחר הכל ✅" if not selected_all else "כל היעדים נבחרו"
        rows.insert(0, [InlineKeyboardButton(label, callback_data="dests:select_all")])
