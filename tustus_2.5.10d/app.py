...
logging.getLogger().info(f'🚀 Running {SCRIPT_VERSION} from {_cwd} (PYTHONPATH={os.environ.get("PYTHONPATH","<unset>")})')
# --- end startup banner ---
...
async def on_error(update: object, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.warning('⚠️ Conflict (another poller?). Ignoring.')
    except RetryAfter as e:
        logger.warning('⏳ RetryAfter: %s', e)
...
async def _post_init(app):
    try:
        ...
        if jq is not None:
            async def _tick(_):
                try:
                    # ensure logic alias exists
                    global lg
                    import importlib
                    lg = lg if 'lg' in globals() else importlib.import_module('logic')
                    await lg.run_monitor(conn, app)
                except Exception:
                    logger.exception('run_monitor tick failed')
            jq.run_repeating(_tick, interval=INTERVAL, first=1.0, name='monitor')
            logger.info('🗓️ JobQueue repeating task scheduled (interval=%ss)', INTERVAL)
        else:
            logger.warning('⚠️ JobQueue לא זמין — נדרש python-telegram-bot[job-queue]')
    except Exception:
...
# --- tustus_2.5.10d: inject sqlite3 connection into Application bot_data ---
try:
    if 'conn' in locals():
        app.bot_data['conn'] = conn  # make DB available to UI/logic
except Exception as _e:
    logging.getLogger(__name__).warning("bot_data DB attach skipped: %s", _e)
# ---------------------------------------------------------------------------
