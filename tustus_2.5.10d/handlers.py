...
# --- tustus_2.5.10d additions: unified home menu ---
def _home_keyboard():
    rows = [
        [InlineKeyboardButton("✈️ כל הטיסות", callback_data="home:all")],
        [InlineKeyboardButton("💵 מחירים", callback_data="home:prices")],
        [InlineKeyboardButton("📍 יעדים", callback_data="home:dests")],
    ]
    return InlineKeyboardMarkup(rows)

async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_chat.send_message("ברוך הבא! בחר מהתפריט:", reply_markup=_home_keyboard())
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "home:all":
            text = lg.ui_all_flights(app)
            await q.message.edit_text(text, reply_markup=_home_keyboard(), disable_web_page_preview=True)
        elif data == "home:prices":
            text = lg.ui_prices(app)
            await q.message.edit_text(text, reply_markup=_home_keyboard(), disable_web_page_preview=True)
        elif data == "home:dests":
            # send destinations with Select All
            kb = lg.ui_dests_keyboard(app)
            await q.message.edit_text("בחר יעדים (או בחר הכל):", reply_markup=kb)
        elif data == "dests:select_all":
            kb = lg.ui_dests_keyboard(app, select_all=True)
            await q.message.edit_text("נבחרו כל היעדים ✅", reply_markup=kb)
        else:
            await q.answer("לא ידוע")
    except Exception:
        logging.exception("callback error")
        if q: await q.answer("שגיאה")
# -------------------------------------------------------------------------
